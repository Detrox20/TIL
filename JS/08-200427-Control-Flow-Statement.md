...
// 식별자의 유효범위
// var 키워드를 선언한 경우 블록문 밖에서도 접근이 가능하다 -> var키워드 부작용
// var 키워드는 블록문을 쓰는 의미가 없다
{
    var x = 1;
    console.log(x); // 1
}
...
결과가 이러한 이유는 case 문에 해당하는 문의 **마지막에 break 문을 사용하지 않았기 때문**이다. break 키워드로 구성된 break 문은 코드 블록에서 탈출하는 역할을 한다. break 문이 없다면 case 문의 표현식과 일치하지 않더라도 실행 순서는 다음 case 문으로 연이어 이동한다. 올바른 switch 문은 아래와 같다.
