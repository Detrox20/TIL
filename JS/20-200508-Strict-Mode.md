...
전역 스코프에도 변수 x의 선언이 존재하지 않기 때문에 ReferenceError를 throw할 것 같지만 자바스크립트 엔진은 암묵적으로 전역 객체에 프로퍼티 x를 동적 생성한다. 이때 전역 객체의 프로퍼티 x는 마치 전역 변수처럼 사용할 수 있다. 이러한 현상을 암묵적 전역(implicit global, [“21.4.3. 암묵적 전역”](https://poiemaweb.com/fastcampus/built-in-object#43-암묵적-전역) 참고)라 한다.
