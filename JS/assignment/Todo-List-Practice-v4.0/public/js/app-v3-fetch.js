...
import { request } from './ajax-promise.js';
...
const getTodos = () => {
  request.get('http://localhost:9000/todos')
    .then(response => {
...
const addTodo = content => {
  ...
  request.post('http://localhost:9000/todos', toBeAddedTodo)
...
const removeTodo = id => {
  request.delete(`http://localhost:9000/todos/${id}`)
    .then(response => response.json())
    .then(_todos => todos = _todos)
    .then(render)
    .catch(err => console.error(err));
...
const toggleTodo = id => {
  ...
  request.patch(`http://localhost:9000/todos/${id}`, changedTodo)
    .then(response => response.json())
    .then(_todos => todos = _todos)
    .then(render)
    .catch(err => console.error(err));
...
const completeAll = completed => {
  request.patch('http://localhost:9000/todos', { completed })
    .then(response => response.json())
    .then(_todos => todos = _todos)
    .then(render)
    .catch(err => console.error(err));
...
const clearCompleted = () => {
  request.delete('http://localhost:9000/todos/completed')
    .then(response => response.json())
    .then(_todos => todos = _todos)
    .then(render)
    .catch(err => console.error(err));
